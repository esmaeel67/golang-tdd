...
func TestListBooksByUser(t *testing.T) {
	...
	t.Run("no book for user", func(t *testing.T) {
		bs := db.NewBookService([]db.Book{}, nil)
		books := bs.ListByUser(uuid.NewString())
		assert.Empty(t, books)
	})
}
...
func TestSwapBook(t *testing.T) {
	...
	t.Run("unknown book ic", func(t *testing.T) {
		bs := db.NewBookService([]db.Book{eb}, nil)
		book, err := bs.SwapBook(uuid.NewString(), uuid.NewString())
		assert.Nil(t, book)
		assert.NotNil(t, err)
	...
	t.Run("unavailable book", func(t *testing.T) {
		...
		book, err = bs.SwapBook(eb.ID, uuid.NewString())
		assert.Nil(t, book)
		assert.NotNil(t, err)
